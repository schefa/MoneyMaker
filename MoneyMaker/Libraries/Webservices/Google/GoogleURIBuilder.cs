...
namespace MoneyMaker.Libraries.Google
{
    ...
    public class GoogleURIBuilder
    {
        ...
        /// </summary>        
        public GoogleURIBuilder(string exchange, string tickerName) {            
        ...
        public string getGetPricesUrlForCurrencyRate()
        {
            return getGetPricesUri(60, "2d");
        }
        ...
        private string getPeriod(DateTime startDate, DateTime endDate) {
            ...
            if (endDate.Year > startDate.Year) { //More than a year.
                result = (endDate.Year - startDate.Year + 1) + "Y";
            }

            TimeSpan span = endDate - startDate;

            if (span.Days > 50) { //Return months.
                result = ((span.Days % 25) + 1) + "M";
            }
            else { //Return days.
                result = (span.Days + 1) + "d"; //+1 because, depending on the hour, no data might be retrieved (1d is for today). 
            }
            return result;
        }
        #endregion
