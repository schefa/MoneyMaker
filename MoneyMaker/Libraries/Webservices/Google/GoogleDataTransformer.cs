...
namespace MoneyMaker.Libraries.Google
{
    ...
    /// <summary>
    /// Processes the raw data from Google and transforms it into something more readable.
    /// </summary>
    public class GoogleDataTransformer : IFinanceDataTransformer
    {

        /// <summary>
        /// Google Finance has no option to get historical currency rates
        /// </summary>
        public List<CurrencyRate> createCurrencyRatesFromString(string csvString)
        {
            return null;
        }

        /// <summary>
        /// Creates a currency rate from string downloaded from Google Finance
        /// </summary>
        public CurrencyRate createCurrencyRateFromString(string csvString)
        {
            CurrencyRate quote = null;
            if (csvString != null)
            {
                string[] elements = csvString.Split(',');
                if (!String.IsNullOrEmpty(elements[0]))
                {
                    quote = new GoogleCurrencyRate()
                    {
                        Date = elements[0],
                        Rate = Convert.ToDouble(elements[4], CultureInfo.InvariantCulture),
                    };
                }
            }
            return quote;
        ...
        /// <summary>
        /// Creates a list of historical quotes from a string downloaded from Google Finance
        /// </summary>
        public List<HistoricalQuote> createHistoricalQuotesFromString(string csvString)
        {
            List<HistoricalQuote> quotes = new List<HistoricalQuote>();
            if(csvString != null)
            { 
                using (MemoryStream ms = new MemoryStream(Encoding.Default.GetBytes(csvString)))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        quotes.Add(new GoogleHistoricalQuote()
                        {
                            Date = Convert.ToDateTime(elements[0]),
                            Open = Convert.ToDecimal(elements[1], CultureInfo.InvariantCulture),
                            High = Convert.ToDecimal(elements[2], CultureInfo.InvariantCulture),
                            Low = Convert.ToDecimal(elements[3], CultureInfo.InvariantCulture),
                            Close = Convert.ToDecimal(elements[4], CultureInfo.InvariantCulture),
                            Volume = Convert.ToInt32(elements[5])
                        });
                    }
                }
            }
        ...
        private int getVolume(string v)
        {
            decimal result;
            int volume = 0;
